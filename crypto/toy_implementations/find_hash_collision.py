...
def find_collision(message):
    ...
    cblocks = []
    for block in get_blocks(new_message, block_size):
        cblocks.append(_counter_mode_inner(block, key, ctr, block_enc))
        if len(cblocks) == 2:
            break
    
    # We need to reset the ctr
    block_size, block_enc, key, ctr = hash_inputs()

    new_block0 = xor_bits(cblocks[1], block_enc(ctr, key)) 
    bits_inc(ctr)  
    new_block1 = xor_bits(cblocks[0], block_enc(ctr, key))   

    new_message[0:block_size] = new_block0
    new_message[block_size:block_size * 2] = new_block1
